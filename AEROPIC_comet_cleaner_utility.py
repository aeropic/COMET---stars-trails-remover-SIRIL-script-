...
def install_dependencies():
    """ Check and install missing dependencies at runtime before main imports """
    # Mapping between pip package name and the name used in 'import'
    dependencies = {
        "opencv-python": "cv2",
        "astropy": "astropy",
        "PyQt6": "PyQt6",
    ...
    for package, import_name in dependencies.items():
        try:
            ...
            print(f"Dependency missing: {package}. Installing...")
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", package])
                print(f"Successfully installed {package}")
            except Exception as e:
                print(f"Failed to install {package}. Error: {e}")
                # We don't exit here to try others, but main imports will fail later if critical.

# Execute check before importing heavy libraries
...
class AEROPIC_Master_Comet_EN(QMainWindow):
    def __init__(self):
        ...
        self.is_ghost_mod_pressed = False 
    ...
    def run_catalog_clean(self):
        ...
        try:
            ...
            while stars_to_process and iteration < 100:
                ...
                for ry, rx in stars_to_process:
                    ...
                    for i in range(self.c):
                        ...
                        val = np.zeros_like(vg); both = clean_g & clean_d; only_g = clean_g & ~clean_d; only_d = clean_d & ~clean_g
            ...
            QMessageBox.information(self, "Processing Complete", f"{total_stars - len(remaining_stars)} stars cleaned.")
        finally:
    ...
    def on_mouse(self, event, x, y, flags, param):
        self.mouse_pos = (x, y); self.is_alt_pressed = bool(flags & cv2.EVENT_FLAG_ALTKEY)
        self.is_ghost_mod_pressed = (flags & cv2.EVENT_FLAG_SHIFTKEY) and (flags & cv2.EVENT_FLAG_CTRLKEY)
        z = self.sld_z.value()/100.0; rx, ry = int((x + self.offset[1])/z), self.h - int((y + self.offset[0])/z)
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.pan_start: self.offset[1]-=(x-self.pan_start[0]); self.offset[0]-=(y-self.pan_start[1]); self.pan_start=(x, y)
        elif event == cv2.EVENT_MOUSEWHEEL:
            if self.is_ghost_mod_pressed: self.ghost_radius = np.clip(self.ghost_radius + (5 if flags > 0 else -5), 5, 300)
            elif self.is_alt_pressed: self.sld_mask_r.setValue(np.clip(self.sld_mask_r.value() + (25 if flags > 0 else -25), 10, 800))
    ...
    def loop(self):
        ...
        if self.chk_show_stars.isChecked() and self.data_stars is not None:
            ...
            for ry, rx in self.detected_coords:
                r_eff = self.special_radii.get((ry, rx), glob_r); color = (0, 165, 255) if (ry, rx) in self.special_radii else (0, 255, 255)
        ...
        if self.is_alt_pressed: cv2.circle(view, self.mouse_pos, int(self.sld_mask_r.value() * z), (0, 0, 255), 2)
        if self.is_ghost_mod_pressed: cv2.circle(view, self.mouse_pos, int(self.ghost_radius * z), (0, 165, 255), 2)
        if self.p1:
